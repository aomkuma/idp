...
@Component({
  selector: 'app-admin-work-info-file-manage',
  templateUrl: './admin-work-info-file-manage.component.html',
  styleUrls: ['./admin-work-info-file-manage.component.scss']
})
export class AdminWorkInfoFileManageComponent implements OnInit {
  ...
  getFileList(){
    this.httpService.callHTTPGet('attach-file/work-info', this.personnel_id).subscribe(
      event => {
  ...
  onSubmit(){
    ...
    this.uploadService.uploadFile('attach-file/work-info', formData).subscribe(
      event => {
        if (event instanceof HttpResponse) {
  ...
  triggerModal(content, data_id) {
    this.modalService.open(content, {ariaLabelledBy: 'modal-basic-title'}).result.then((res) => {
      ...
      if(res == 'Confirm'){
        ...
        this.httpService.callHTTPDelete('attach-file/work-info/destroy/', data_id).subscribe(
          event => {
            if (event instanceof HttpResponse) {
              this.getFileList();
            }
