...
@Component({
  selector: 'app-admin-evaluate-form',
  templateUrl: './admin-evaluate-form.component.html',
  styleUrls: ['./admin-evaluate-form.component.scss']
})
export class AdminEvaluateFormComponent implements OnInit {
  ...
  hrSave() {
    ...
    this.uploadService.uploadFile('evaluate-form/hr/mark-status', formData).subscribe(
      event => {
        if (event instanceof HttpResponse) {
          // console.log(event['body']['id']);
          this.submitted = false;
